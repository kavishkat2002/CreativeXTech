/**
 * Seed script — run once to push all existing articles into Supabase.
 *
 * Usage:
 *   node --env-file=.env.local scripts/seed-articles.mjs
 */
...
// Use native fetch directly for maximum compatibility
...
async function seed() {
  ...
  try {
    const data = await upsertArticles(articles);
